...
Original file is located at
    https://colab.research.google.com/drive/1hnrSD1daUeMXcrzI1eH89y1PySfhIpFy
"""
...
model = TFAutoModelForSequenceClassification.from_pretrained(bert, num_labels=2, id2label={0: 'non-ofensive', 1: 'ofensive'})
...
# model.fit(train_encodings, np.array(train_labels), validation_data=(val_encodings, np.array(test_labels)), epochs=maximo_epocas, batch_size=size, callbacks=[early_stopping])
...
pred_labels = pipe(test_texts.tolist())
bert_labels = [item['label'] for item in pred_labels]

file_name = './drive/MyDrive/Dataset/pred_labels.json'

# Verificando os dados carregados


# 1: fake; 0: opinion; 0: Não ofensivo
from sklearn.metrics import classification_report


# 1: fake; 0: opinion; -1 e/ou 2: news
bert_labels = [label.replace('non-ofensive', '0') for label in bert_labels]
bert_labels = [label.replace('ofensive', '1') for label in bert_labels]
